...
class Spider_Registration_Prostate(SessionSpider):
    ...
    def __init__(self, spider_path, jobdir,
        ...
        # Check Executable:
        self.reg_aladin_exe = self.check_executable(ARGS.reg_aladin_exe,
                                                    'reg_aladin')
        self.reg_f3d_exe = self.check_executable(ARGS.regf3d_exe, 'reg_f3d')
    ...
    def make_pdf(self):
        ...
        pdf_pages = {}
        page_number = 1
        pdf_title = 'Registration T2 - ADC/DCE Prostate'
        for scan_id, res_dict in self.sources.items():
            labels = {'0': 'Target - %s - %s' %
                           (self.target['ID'],
                            self.target['type']),
            ...
            # Get the images path and the label associated:
            images = [self.target['nii'], res_dict['nii']]
