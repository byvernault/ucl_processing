...
def parse_args():
    ...
    '''
    ap = spiders.get_scan_argparser("Vessel_Extraction", "Extract the vessel from the T1/MPRAGE scan")
    ap.add_argument("--vesselExtPath", dest="vessel_extractor", help="path to the executable niftkVesselExtractor.", required=True)
...
class Spider_Vessel_Extraction(ScanSpider):
    ...
    def __init__(self, spider_path, jobdir, xnat_project, xnat_subject, xnat_session, xnat_scan,
        ...
        super(Spider_Vessel_Extraction, self).__init__(spider_path, jobdir, xnat_project, xnat_subject,
        ...
        pversion = sb.Popen([ARGS.vessel_extractor, '--version'], stdout=sb.PIPE,
                                                                  stderr=sb.PIPE)
        nve_version, _ = pversion.communicate()
        self.time_writer('niftkVesselExtractor version: %s' % (nve_version.strip()))

    def get_voxel_size(self):
        '''
            Method to get the voxel size from XNAT if define
             using default value if not.
    ...
    def pre_run(self):
        ...
        # Read from XNAT the pixel size from DICOM, if not set to default value:
        vsize = self.get_voxel_size()
        if not vsize:
            self.time_writer("Using default value for pixel size in niftkVesselExtractor. Value not set on XNAT.")
        else:
    ...
    def run(self, vessel_extractor):
        ...
        if not os.path.exists(exe_path):
            ...
            output_name = "%s_vessel_extracted.nii.gz" % (os.path.basename(self.inputs[0]).split('.')[0])
