"""Processor associated to Spider_Registration2Ref.

Author:         Benjamin Yvernault
contact:        b.yvernault@ucl.ac.uk
Processor name: Processor_Registration2Ref
Creation date:  2016-02-24 14:06:18.168057
Purpose:        Register scans from a session with a specific types to one
                reference scan (same session)
"""
...
__purpose__ = "Register scans from a session with a specific types to one \
reference scan (same session)"
...
ucl_spiders/Spider_Registration2Ref_v1_0_0.py')
...
--scanRef {target} \
--regAladin {regaladin} \
--openm_core {number_core}
"""


class Processor_Registration2Ref(SessionProcessor):
    ...
    def __init__(self, spider_path=DEFAULT_SPIDER_PATH, version=None,
                 ...
                 ppn=DEFAULT_PPN, target_type=None, sources_type=DEFAULT_TYPES,
                 reg_aladin_exe=DEFAULT_REG_ALADIN, suffix_proc=''):
        """Entry point for Processor_Registration2Ref Class."""
        super(Processor_Registration2Ref,
              self).__init__(walltime, mem_mb, spider_path, version, ppn=ppn,
                             suffix_proc=suffix_proc)
        # Reference
        if not target_type:
            raise Exception("Processor_Registration2Ref: \
reference type not set. Please edit your settings file.")
        self.target_type = target_type
        # Images to register
        if isinstance(sources_type, list):
            self.sources_type = sources_type
        elif isinstance(sources_type, str):
            self.sources_type = sources_type.split(',')
        if not sources_type:
            raise Exception("Processor_Registration2Ref: \
scantypes to register not set. Please edit your settings file.")
        # Reg Aladin
        if not os.path.exists(reg_aladin_exe):
            raise Exception("Processor_Registration2Ref: \
reg_aladin path given not found: %s." % reg_aladin_exe)
